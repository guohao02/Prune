...
parser.add_argument("--loss-type", "-loss", dest="loss",default='zol',
                    choices=list(LossType.loss_name().keys()), help="the type of loss")
parser.add_argument('--lbd', type=float, default=3e-5,
...
parser.add_argument('--save', type=str, metavar='PATH', default='./checkpoints',
                    help='path to save prune model')
parser.add_argument('--arch', default='vgg16_linear', type=str,
                    help='architecture to use')
parser.add_argument('--log', type=str, metavar='PATH',default='./events',
...
parser.add_argument('--bn-wd', type=int, default=128, metavar='N',
...
if args.dataset == 'cifar10':
    train_loader = torch.utils.data.DataLoader(
        datasets.CIFAR10('./cifar/data.cifar10', train=True, download=True,
    ...
    test_loader = torch.utils.data.DataLoader(
        datasets.CIFAR10('./cifar/data.cifar10', train=False, transform=transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
        ])),
...
def updateBN():
    if args.loss == LossType.L1_SPARSITY_REGULARIZATION:
        ...
        bn_modules = list(filter(lambda m: (isinstance(m[1], nn.BatchNorm2d) or isinstance(m[1], nn.BatchNorm1d)),
                                 model.named_modules()))#filter() 函数用于过滤序列，过滤掉不符合条件的元素，返回由符合条件元素组成的新列表
        bn_modules = list(map(lambda m: m[1], bn_modules))#remove module name,获取value
        for m in bn_modules:
            if isinstance(m, nn.BatchNorm2d) or isinstance(m, nn.BatchNorm1d):
                m.weight.grad.data.add_(sparsity * torch.sign(m.weight.data))#更新梯度
    else:
...
def clamp_bn(model, lower_bound=0, upper_bound=1):
    ...
    for m in sparse_modules:
        m.weight.data.clamp_(lower_bound, upper_bound)#clamp_():钳位，将输入限制在这个范围内
        # in-place类型是指，但在一个tensor上操作了之后，是直接修改了这个tensor，还是返回一个新的tensor，而旧的tensor并不修改。对tensor的一个函数后加上了下划线，则表明这是一个in-place类型
        ...
        if flops_weighted:
            for sub_module in model.modules():
                if isinstance(sub_module, model.building_block):
                    ...
                    for sparse_m, flops_w in zip(sub_module_sparse_layers, flops_weight):#zip() 函数用于将可迭代的对象作为参数，将对象中对应的元素打包成一个个元组，然后返回由这些元组组成的列表。
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        # logging
        avg_loss += loss.data.item()#item()得到元素值
        pred = output.data.max(1, keepdim=True)[1] # 这个会返回他在dim=1的最大值和最大值的位置中的最大值
        train_acc += pred.eq(target.data.view_as(pred)).cpu().sum()#view_as()该函数的作用是将调用函数的变量，转变为同参数tensor同样的形状。
