...
class SparseGate(nn.Module):
    ...
    def forward(self, x):
        ...
        out: torch.Tensor = self._conv(x)#: torch.Tensor为数据标注
    ...
    def weight(self) -> torch.nn.Parameter:
        ...
        """
        return self._conv.weight#输出卷积层权值
    ...
    def do_pruning(self, mask: np.ndarray):
        #np.argwhere(a)返回非0的数组元组的索引，或者其中a是要索引数组的条件
        idx = np.squeeze(np.argwhere(np.asarray(mask)))#从数组的形状中删除单维条目，即把shape中为1的维度去掉
        if len(idx.shape) == 0:
            # expand the single scalar to array，单个标量展开为数组
            idx = np.expand_dims(idx, 0)#np.expand_dims用于扩展数组的形状
        ...
        self._conv.weight.data = self._conv.weight.data.clone()[idx.tolist(), :, :, :]#tolist()将数组或者矩阵转换成列表
...
def prune_conv_layer(conv_layer: Union[nn.Conv2d, nn.Linear],
    ...
    with torch.no_grad():
        ...
        idx_in = np.squeeze(np.argwhere(np.asarray(in_channel_mask)))#np.argwhere(a):返回非0的数组元组的索引，其中a是要索引数组的条件。
        ...
        if isinstance(conv_layer, nn.Conv2d):
            if conv_layer.groups == 1:
                a = conv_weight
                conv_weight = conv_weight[:, idx_in.tolist(), :, :]#tolist():将数组或者矩阵转换成列表
                print(a - conv_weight)
            else:
