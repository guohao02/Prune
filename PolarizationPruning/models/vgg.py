...
class VGGBlock(BuildingBlock):
    ...
    @property# @property可以把一个方法装饰成一个属性来使用的. 这是它的语法含义.① staticmethod ② classmethod ③ property
    ...
    def __init__(self, gate: bool, dataset='cifar10', depth=19, init_weights=True, cfg: List[int] = None, linear=False,
        ...
        if cfg is None:
            ...
            for i in range(len(cfg)):
                if cfg[i] == 'M':
                    continue
                cfg[i] = max(1, int(cfg[i] * width_multiplier))#返回最大值

        if linear:
            self.feature = self.make_layers(cfg[:-1], True)#cfg[:-1]去除了这行文本的最后一个字符（换行符）后剩下的部分
        else:
