...
def prune_vgg16_conv_layer(model, layer_index, filter_index):
    ...
    while layer_index + offset < len(model.features._modules.items()):
        # print(len(model.features._modules.items()))
    ...
    new_conv.weight.data = torch.from_numpy(new_weights).cuda()#torch.from_numpy()从numpy.ndarray创建一个张量
...
def replace_layers(model, i, indexes, layers):
    if i in indexes:
        return layers[indexes.index(i)]# index() 方法检测字符串中是否包含子字符串 str 
    return model[i]
...
if __name__ == '__main__':
    ...
    t0 = time.time()
    model = prune_vgg16_conv_layer(model, 26, 10)
