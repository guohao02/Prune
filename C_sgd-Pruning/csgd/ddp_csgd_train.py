...
def cluster_by_kmeans(kernel_value, num_cluster):
    ...
    for j in range(num_cluster):
        result.append([])
    for i, c in enumerate(km.labels_):#km.labels_：聚类标签
        result[c].append(i)
    for r in result:
        assert len(r) > 0
    return result
...
def _is_follower(layer_idx, pacesetter_dict):
    followers_and_pacesetters = set(pacesetter_dict.keys())#set():函数创建一个无序不重复元素集，可进行关系测试，删除重复数据，还可以计算交集、差集、并集等;keys():键值
...
save_hdf5_epochs=10000):
    ...
    ensure_dir(cfg.output_dir)#create directories if *path* does not exist
    ...
    with Engine(local_rank=local_rank) as engine:
        ...
        if os.path.exists(clusters_save_path):
            layer_idx_to_clusters = np.load(clusters_save_path, allow_pickle=True).item()#读取文件
        else:
            if local_rank == 0:
                layer_idx_to_clusters = get_layer_idx_to_clusters(kernel_namedvalue_list=kernel_namedvalue_list,
                                                                  target_deps=target_deps,
                                                                  pacesetter_dict=pacesetter_dict)#target_deps：每层要保留的卷积核数;kmeans聚类
        ...
        # ------------------------------------------pruning------------------------------------
        # 如果存在training文件，则直接pruning
        if os.path.exists(os.path.join(cfg.output_dir, 'cgsd_finish.hdf5')):
            if local_rank == 0:
        ...
        engine.save_hdf5(os.path.join(cfg.output_dir, 'cgsd_finish.hdf5'))
    ...
    # ------------------------------------------pruning------------------------------------
    # pruned_weights:cfg.output_dir + pruning.hdf5 
    if local_rank == 0:
        csgd_prune_and_save(engine=engine, layer_idx_to_clusters=layer_idx_to_clusters,
                        save_file=pruned_weights, succeeding_strategy=succeeding_strategy, new_deps=target_deps)