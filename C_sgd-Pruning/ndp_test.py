...
def run_eval(val_data, max_iters, net, criterion, discrip_str, dataset_name):
    pbar = tqdm(range(max_iters))#Tqdm 是一个快速，可扩展的Python进度条，可以在 Python 长循环中添加一个进度提示信息，用户只需要封装任意的迭代器 tqdm(iterator)。
    top1 = AvgMeter()#实例化
    top5 = AvgMeter()
    losses = AvgMeter()
...
def val_during_train(epoch, iteration, tb_tags,
    ...
    model.eval()
    num_examples = num_val_examples(dataset_name)#获取数据个数
    assert num_examples % test_batch_size == 0#整除
    val_iters = num_examples // test_batch_size#遍历测试一次的次数
    eval_dict, total_net_time = run_eval(val_data, val_iters, model, criterion, descrip_str,
                                         dataset_name=dataset_name)#{top1,top5，loss}，网络运行时间
