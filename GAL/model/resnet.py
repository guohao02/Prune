...
class SparseResBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1):
        ...
        m = Normal(torch.tensor([norm_mean]), torch.tensor([norm_var])).sample()#返回一个张量，包含从给定参数means,std的离散正态分布中抽取随机数
        self.mask = Mask(m)
...
class ResNet(nn.Module):
    def __init__(self, block, num_layers, num_classes=10, has_mask=None):
        ...
        n = (num_layers - 2) // 6# resnet56为9
        
        if has_mask is None : 
            has_mask = [1]*3*n#长度为27，value为1的向量
            # print(has_mask)
    ...
    def _make_layer(self, block, planes, blocks, stride, has_mask):
        ...
        if not has_mask[0] == 0:
            layers.append(block(self.inplanes, planes, stride))# has_mask[0]：构造layers

        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            if not has_mask[i] == 0:
                layers.append(block(self.inplanes, planes))# has_mask[1:blocks-1]:构造layers
